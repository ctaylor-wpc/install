...
def _sig_bytes_from_canvas(customer_signature):
    """Extract PNG bytes from a canvas result object. Returns BytesIO or None."""
    try:
        if customer_signature is None or not hasattr(customer_signature, 'image_data'):
            return None
...
def _sig_bytes_from_base64(sig_b64):
    """Decode a base64 signature string to a PNG BytesIO. Returns BytesIO or None."""
    try:
        if not sig_b64:
            return None
...
def _place_signature_in_doc(doc, sig_bytes):
    """
    Try to place sig_bytes (PNG BytesIO) into the customer_signature widget field.
    Returns (signature_field_found, signature_placed).
    """
    ...
    if signature_field_found and not signature_placed:
        # Fallback: stamp onto last page at hardcoded coords
        page = doc[-1]
    ...
    return signature_field_found, signature_placed
...
def generate_pdf(plants_data, installation_data, customer_data, pricing_data, customer_signature=None):
    try:
        ...
        # Resolve signature bytes: canvas drawing takes priority, then stored base64.
        sig_bytes = _sig_bytes_from_canvas(customer_signature)
        if sig_bytes is None:
            sig_bytes = _sig_bytes_from_base64(customer_data.get('signature_base64'))
        ...
        if sig_bytes is not None:
            try:
                signature_field_found, signature_placed = _place_signature_in_doc(doc, sig_bytes)
