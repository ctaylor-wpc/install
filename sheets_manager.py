...
def _get_or_create_state_sheet(workbook):
    """Get the Install States worksheet, creating it with headers if needed."""
    try:
        state_sheet = workbook.worksheet("Install States")
    except:
        state_sheet = workbook.add_worksheet(title="Install States", rows=1000, cols=20)
        headers = [
            "Install ID", "Customer Name", "Date Saved",
            "Plants Data", "Installation Data", "Customer Data", "Pricing Data",
            "PDF Link", "Record Type"
        ]
        state_sheet.append_row(headers)
    return state_sheet


def save_install_state(install_id, plants_data, installation_data, customer_data, pricing_data, pdf_link="", record_type="install"):
    """
    Save complete install state to the Install States tab.
    record_type is either "install" or "quote".
    Updates existing row if install_id already exists.
    """
    try:
        client = get_gspread_client()
        workbook = client.open_by_key(SHEET_ID)
        state_sheet = _get_or_create_state_sheet(workbook)
        ...
        if row_to_update:
            state_sheet.update(f'A{row_to_update}:I{row_to_update}', [row_data], value_input_option='USER_ENTERED')
        else:
...
def load_install_states():
    ...
    Returns a list of dicts. Includes record_type field ("install" or "quote").
    """
    try:
        client = get_gspread_client()
        workbook = client.open_by_key(SHEET_ID)
        ...
        for record in records:
            try:
                installs.append({
                    ...
                    'pdf_link': record.get('PDF Link', ''),
                    # Older rows won't have Record Type; default to "install"
                    'record_type': record.get('Record Type', 'install') or 'install',
                })
